...
function DemoButton({ inverse = false }: { inverse?: boolean }) {
  return <Button asChild size="lg" variant={inverse ? "secondary" : "outline"}><a href="mailto:hello@logitrack.example?subject=LogiTrack%20Demo">Request a Demo <ArrowRight /></a></Button>;
}
...
function LogiTrackPage() {
  return <div id="top" className="min-h-screen bg-background text-foreground"><Nav /><main>
    ...
    <section id="overview" className="section-pad"><div className="site-container"><SectionHeader centered label="Your business. One view." title="See your transport business at a glance." copy="From active trips and revenue to expenses and profitability, LogiTrack gives you the information you need to understand your business without piecing it together yourself." /><div className="mt-12"><ScreenshotFrame label="LogiTrack Dashboard" alt="Placeholder for the actual LogiTrack dashboard screenshot showing a business overview" /></div><div className="mt-10 grid divide-y border-y border-border md:grid-cols-4 md:divide-x md:divide-y-0">{([{ icon: CircleDollarSign, title: "Trip profitability", copy: "Know what every trip made." },{ icon: WalletCards, title: "Customer payments", copy: "Know what's collected and pending." },{ icon: Truck, title: "Vehicle costs", copy: "Know what your trucks are costing." },{ icon: Users, title: "Driver payments", copy: "Know what's paid and payable." }]).map(({ icon: Icon, title, copy }) => <div key={title} className="outcome-block"><Icon /><h3>{title}</h3><p>{copy}</p></div>)}</div><p className="mt-12 text-center text-lg font-medium">Once you can see the business, you can understand where the money is being made.</p></div></section>
